...
class TAWAP {
    public int[] solution(String[] park, String[] routes) {
        int[] answer = {0, 0}; // 시작 위치를 park의 S로 초기화
        ...
        for (String route : routes) {
            ...
            boolean canMove = true; // 장애물이나 크기를 넘어가면 행동을 취소하기위함
            ...
            for (int i = 0; i < distance; i++) {
                ...
                if (nextRow < 0 || nextRow >= park.length || nextCol < 0 || nextCol >= park[0].length() || park[nextRow].charAt(nextCol) == 'X') {
                    canMove = false;
                    break;
                }
            }
...
public class TakeAWalkAtPark {
    public static void main(String[] args){
        TAWAP tawap = new TAWAP();
        System.out.println(tawap.solution(new String[]{"SOO","OOO","OOO"}, new String[]{"E 2","S 2","W 1"}));
    }
}
