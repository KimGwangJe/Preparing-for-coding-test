        ...
        for (int i = 0; i < targets.length; i++) {
            ...
            for (int j = 0; j < targets[i].length(); j++) {
                ...
                for (int k = 0; k < keymap.length; k++) {
                    int charIndex = keymap[k].indexOf(targetChar);
                    if (charIndex != -1 && charIndex < minKeyPress) {
                        keyIndex = k;
                        minKeyPress = charIndex;
                    }
                }
...
public class MadeKeyboard {
    public static void main(String[] args){
        MKB mkb = new MKB();
        System.out.println(mkb.solution(new String[]{"ABACD","BCEFD"},new String[]{"ABCD","AABB"}));
    }
}
