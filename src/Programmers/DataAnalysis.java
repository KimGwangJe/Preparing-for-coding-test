...
class DA {
    public int[][] solution(int[][] data, String ext, int val_ext, String sort_by) {
        ...
        if (ext.equals("code")) select = 0;
        else if (ext.equals("date")) select = 1;
        else if (ext.equals("maximum")) select = 2;
        else select = 3;

        for (int i = 0; i < data.length; i++) {
            if (data[i][select] < val_ext) size++;
        }
        ...
        // 람다식을 사용하여 정렬
        Arrays.sort(answer, (o1, o2) -> Integer.compare(o1[getSortIndex(sort_by)], o2[getSortIndex(sort_by)]));
...
public class DataAnalysis {
    public static void main(String[] args){
        DA da = new DA();
        System.out.println(da.solution(new int[][]{{1, 20300104, 100, 80},{2, 20300804, 847, 37},{3, 20300401, 10, 8}},"data",20300501,"remain"));
    }
}
