...
공격력이 4인 무기를 구매합니다. 만약, 이웃나라와의 협약으로 정해진 공격력의 제한수치가 3이고 제한수치를 초과한 기사가 사용할 무기의 공격력이 2라면,
...
class TW {
    public int solution(int number, int limit, int power) {
        ...
        for(int i = 1; i <= number; i++){ // number 반복
            int temp = 0; // 약수 갯수
            for(int j = 1; j <= number / i; j++){ // 약수
                if(i%j==0){
                    temp++;
                }
                if(j == i / 2){
...
public class TemplarWeapon {
    public static void main(String[] args){
        TW tw = new TW();
        System.out.println(tw.solution(5,3,2));
    }
}
