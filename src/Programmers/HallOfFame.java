...
* 즉 프로그램 시작 이후 초기에 k일까지는 모든 출연 가수의 점수가 명예의 전당에 오르게 됩니다. k일 다음부터는 출연 가수의 점수가
* 기존의 명예의 전당 목록의 k번째 순위의 가수 점수보다 더 높으면, 출연 가수의 점수가 명예의 전당에 오르게 되고 기존의 k번째 순위의 점수는 명예의 전당에서 내려오게 됩니다.
...
class HOF {
    public int[] solution(int k, int[] score) {
        int[] answer = new int[score.length]; //정답
...
public class HallOfFame {
    public static void main(String[] args){
        HOF hof = new HOF();
        hof.solution(3, new int[]{10, 100, 20, 150, 1, 100, 200});
    }
}
