 ...
 * XYZ 마트에서 14일간 회원을 대상으로 할인하는 제품이 날짜 순서대로 치킨, 사과, 사과, 바나나, 쌀,
 * 사과, 돼지고기, 바나나, 돼지고기, 쌀, 냄비, 바나나, 사과, 바나나인 경우에 대해 알아봅시다.
...
class DP {
    public int solution(String[] want, int[] number, String[] discount) {
        int answer = 0;
        int num = Arrays.stream(number).sum();

        for (int i = 0; i <= discount.length - num; i++) {
            boolean t = true;
            int[] li = Arrays.copyOf(number, number.length);
...
public class DiscountPromotion {
    public static void main(String[] args){
        DP dp = new DP();
        System.out.println(dp.solution(new String[]{"banana", "apple", "rice", "pork", "pot"}, new int[]{3,2,2,2,1}, new String[]{"chicken", "apple", "apple", "banana", "rice", "apple", "pork", "banana", "pork", "rice", "pot", "banana", "apple", "banana"}));
    }
}
